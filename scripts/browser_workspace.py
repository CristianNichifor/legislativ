...
def route(stare, route, qs, body, method="GET"):
    ...
    if method == "POST":
        ...
        handlers = {
            ...
            "/reguli": lambda: rule_candidate_queue.lista(
                path, ident, offset, get("bucket", "all")
            ),
            "/rule-drafts": lambda: law_rule_drafts.lista(
                path,
                ident,
                offset,
                act_id=get("act", ""),
                provision_id=get("provision_id", ""),
            ),
            "/rule-drafts/checks": lambda: law_rule_execution.delegated_norms(
                stare,
                path,
                ident,
                act_id=get("act", ""),
