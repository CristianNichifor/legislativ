...
(async () => {
  ...
  try {for (const width of [1280, 390]) {
    ...
    assert.equal(await page.locator('#browser-generation [data-check]').innerText(), 'Cauta versiune noua');
    assert.doesNotMatch(await page.locator('#browser-generation').innerText(), /reincarca|rebuild|index/i);
    ...
    await mode(page, 'a');
    await page.locator('#browser-generation [data-check]').click(); await idle(page);
    assert.equal(await page.locator('#browser-generation [data-select]').innerText(), 'Alege versiunea online');
    ...
    assert.equal(await page.locator('#browser-generation [data-reload]').innerText(), 'Aplica datele in pagina');
