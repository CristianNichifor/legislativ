...
def test_matrix_workspace_queue_renders_daily_items_and_routes_actions():
    ...
    program = (
        ...
        "const draftButtons=[{dataset:{matrixWorkspaceDraft:'Ministerul Test'},onclick:null}];"
        "const ruleButtons=[{dataset:{matrixWorkspaceRule:'Ministerul Test'},onclick:null}];"
        ...
        "assert.ok(calls.some(c=>c[0]==='scroll'&&c[1]==='ai'));"
        "ruleButtons[0].onclick();"
        "assert.ok(status.textContent.includes('regula candidată'));"
