...
def test_source_coverage_renderer_shows_blockers_and_escapes():
    ...
    program = (
        ...
        "states:{failed:1,changed:1},fetched:1,changed:1,failed:1,incomplete:1,"
        "last_checked:'2026-09-12T00:00:00Z',next_action:'Reîncearcă <b>'}],"
        "blockers:[{kind:'x',message:'Lipsă <script>'}],"
        "limitari:['Doar local <img>']});"
        "assert.ok(h.includes('Control center surse'));"
        "assert.ok(h.includes('Acoperire incompletă'));"
        "assert.ok(h.includes('Camera &lt;x&gt;'));"
        "assert.ok(h.includes('Suport necunoscut'));"
        "assert.ok(h.includes('Eșuat: 1'));"
        "assert.ok(h.includes('1 citite'));"
        "assert.ok(h.includes('1 schimbate'));"
        "assert.ok(h.includes('1 eșuate/indisponibile'));"
        "assert.ok(h.includes('Ultima verificare: 2026-09-12T00:00:00Z'));"
        "assert.ok(h.includes('Reîncearcă &lt;b&gt;'));"
        "assert.ok(h.includes('data-source-control-open=\"camera\"'));"
        "assert.ok(h.includes('data-source-control-attention=\"camera\"'));"
        "assert.ok(!h.includes('data-source-control-note=\"camera\"'));"
        ...
        "h=sourceCoverageFamilyHtml({family:'ue_cellar',label:'Drept UE',required:true,"
        "status:'missing',total:0,fetched:0,changed:0,failed:0,incomplete:0,"
        "states:{},next_action:'Adaugă CELEX',support:{label:'Suportată',"
        "policy:'Poate fi urmărită incremental.'},actions:[{key:'add_source',"
        "label:'Adaugă sursă',target:'ue_cellar',enabled:true},{key:'open_family',"
        "label:'Deschide familia',target:'ue_cellar',enabled:true},"
        "{key:'create_note',label:'Creează notă din lipsă/eșec',target:'ue_cellar',"
        "enabled:true}]});"
        "assert.ok(h.includes('Suportată'));"
        "assert.ok(h.includes('Poate fi urmărită incremental.'));"
        "assert.ok(h.includes('data-source-control-add=\"ue_cellar\"'));"
        "assert.ok(h.includes('data-source-control-note=\"ue_cellar\"'));"
        "const note=sourceCoverageFamilyNote({family:'ue_cellar',label:'Drept UE',"
        "status:'missing',support:{label:'Suportată',policy:'policy'},total:0,"
        "fetched:0,changed:0,failed:0,incomplete:0,next_action:'Importă CELEX'});"
        "assert.equal(note.type,'risc_ue');"
        "assert.equal(note.status,'needs_evidence');"
        "assert.ok(note.evidence_quote.includes('Importă CELEX'));"
    )
    result = subprocess.run(["node", "-e", program], capture_output=True, timeout=10)
    assert result.returncode == 0, result.stderr.decode()
...
def test_source_registry_renderer_shows_econsultare_snapshot_and_registration_copy():
    ...
    program = (
        ...
        "unsupported_full_text:false,publication_reference:{lifecycle_state:'published_monitor',"
        "status:'published_reference',full_text:{label:'Text Monitor nelocalizat'},"
        "limitations:['Referința este metadata-first; nu descarcă Monitorul Oficial.']}}}];"
        "h=sourceRegistryDetailHtml(row,families);"
        "assert.ok(h.includes('Referință Monitorul Oficial'));"
        "assert.ok(h.includes('Număr Monitor'));"
        "assert.ok(h.includes('390'));"
        "assert.ok(h.includes('published_monitor'));"
        "assert.ok(h.includes('published_reference'));"
        "assert.ok(h.includes('Text Monitor nelocalizat'));"
        "assert.ok(h.includes('publication_tracker'));"
        "assert.ok(h.includes('nu descarcă Monitorul Oficial'));"
...
def test_operational_tracker_renderers_show_actions_filters_and_public_feed():
    ...
    program = (
        ...
        "url:'javascript:alert(1)',needs_attention:true,family:'consultare_minister',"
        "family_label:'Consultări ministere',missing_fields:['deadline'],"
        "next_action:'Completează <x>'}]});"
        "assert.ok(h.includes('Consultare &lt;b&gt;'));"
        "assert.ok(h.includes('Deschisă'));"
        "assert.ok(h.includes('Consultări ministere'));"
        "assert.ok(h.includes('lipsesc: deadline'));"
        "assert.ok(h.includes('Completează &lt;x&gt;'));"
        "assert.ok(h.includes('Sincronizează sursa'));"
        "assert.ok(h.includes('Timeline'));"
        "assert.ok(h.includes('data-econsultare-timeline=\"consultare_minister\"'));"
...
def test_lifecycle_renderer_filters_and_opens_project_sources():
    ...
    program = (
        ...
        "timeline_coverage:{total_events:2,missing:['publication'],next_action:'Completează <x>',"
        "next_source_hint:'Verifică Monitor <x>',next_missing_stage:{label:'Monitor'},"
        "evidence_counts:{documents:1,committees:1,reports:1,opinions:1,votes:1},"
        "document_urls:['https://www.cdep.ro/raport.pdf'],"
        "stages:[{key:'committee',label:'Comisie',state:'present',count:1},"
        "{key:'publication',label:'Monitor',state:'missing',count:0}]},"
        ...
        "assert.ok(h.includes('data-lifecycle-event-coverage'));"
        "assert.ok(h.includes('Dovezi tracker: 2 evenimente locale'));"
        "assert.ok(h.includes('1 etape fără dovadă locală'));"
        ...
        "assert.ok(h.includes('Dovezi parlamentare:'));"
        "assert.ok(h.includes('1 documente'));"
        "assert.ok(h.includes('1 rapoarte'));"
        "assert.ok(h.includes('1 avize'));"
        "assert.ok(h.includes('1 voturi'));"
        "assert.ok(h.includes('document 1'));"
        ...
        "evidence:3,deadline:0,committee:3,vote:0,published:0,unknown:0,unavailable:1,registry:1,"
        "affected:1});"
        ...
        "filter_buckets:{attention:1,evidence:6,deadline:2,committee:3,vote:4,published:5,stale:1,"
        ...
        "assert.ok(overview.includes('6 dovezi lipsă'));"
        ...
        "assert.ok(overview.includes('Etapă &lt;nouă&gt;'));"
        "assert.ok(overview.includes('Titlu &lt;x&gt;'));"
        "assert.ok(overview.includes('Mapează &lt;x&gt;'));"
...
def test_lifecycle_renderer_exposes_phase_b_work_buckets():
    ...
    program = (
        ...
        "const evidence={...base,project_id:'PL-x 8',filter_buckets:['evidence'],"
        "timeline_coverage:{total_events:1,missing:['vote']}};"
        ...
        "h=lifecycleListHtml({projects:[base,evidence]},'evidence');"
        "assert.ok(h.includes('PL-x 8')&&!h.includes('PL-x 0'));"
        ...
        "assert.deepEqual(lifecycleCounts({projects:[base,deadline,committee,vote,published,evidence]}),"
        "{watched:0,attention:0,evidence:1,deadline:1,committee:1,vote:1,published:1,stale:0,"
        "unknown:0,unavailable:0,registry:0,affected:0});"
        "const overview=lifecycleOverviewHtml({returned:5,total:5,projects:[base],"
        "filter_buckets:{attention:0,evidence:6,deadline:2,committee:3,vote:4,published:5,stale:0,"
        "unknown:0,unavailable:0,affected:0}});"
        "assert.ok(overview.includes('Cozi de lucru'));"
        "assert.ok(overview.includes('6 dovezi lipsă'));"
        "assert.ok(overview.includes('2 termene'));"
...
def test_matrix_drilldown_renderer_exposes_sources_and_authoring_actions():
    ...
    program = (
        ...
        "evidence_records:[{id:'ev-1',kind:'lacuna',label:'Lacună candidată',act_id:'lege-1',"
        "locator:'art1',quote:'Text citat <quote>',source_url:'https://legislatie.just.ro/x',"
        "source_hash:'a'.repeat(64),parser_version:'legislatie_ro.v1',"
        "uncertainty:'completă pentru revizie locală',not_legal_verdict:true},"
        "{id:'ev-2',kind:'lacuna',label:'Dovadă incompletă <x>',act_id:'lege-2'}],"
        "related_items:{contract:'matrix-related-items-v1',not_legal_verdict:true,laws:[{"
        "act_id:'lege-1',label:'Legea 1',title:'Titlu lege',url:'https://legislatie.just.ro/x',"
        "source_state:'fetched',source_hash:'a'.repeat(64)}],projects:[{plx_id:'PL-x 1/2026',"
        "title:'Proiect',stage:'raport',url:'https://www.cdep.ro/p'}],eu:[{celex:'32014L0024',"
        "imported:false,mentions:2,command:'python -m import <x>'}],monitor:[{project_id:'lege-1',"
        "source_url:'https://monitoruloficial.ro/x',occurred_at:'2026-01-01',source_hash:'b'.repeat(64)}],"
        "consultations:[{project_id:'lege-1',label:'Consultare deschisă <x>',"
        "event_type:'public_consultation_opened',source_url:'https://e-consultare.gov.ro/x',"
        "occurred_at:'2026-02-01',source_hash:'c'.repeat(64)}],"
        "notes:[{title:'Notă existentă',type:'lacuna',status:'ready_for_review',act_id:'lege-1',"
        "locator:'art1'}],proposals:[{title:'Propunere existentă',act_id:'lege-1',locator:'art1'}],"
        "rules:[{candidate_id:'rule-1',status:'reviewable',review_state:'machine_detected',"
        "act_id:'lege-1',locator:'art1'}]},"
        ...
        "assert.ok(h.includes('Se preia în dosar · gata pentru notă'));"
        "assert.ok(h.includes('Țintă:'));"
        "assert.ok(h.includes('Dovadă:'));"
        "assert.ok(h.includes('citat disponibil · sursă identificată'));"
        "assert.ok(h.includes('Poți crea notă, redactare sau ciornă AI'));"
        "assert.ok(h.includes('Se preia în dosar · incompletă'));"
        "assert.ok(h.includes('Completează înainte de concluzie: citat exact, URL sau SHA-256.'));"
        "assert.ok(!h.includes('Dovadă incompletă <x>'));"
        ...
        "assert.ok(h.includes('Monitorul Oficial'));"
        "assert.ok(h.includes('Consultări publice'));"
        "assert.ok(h.includes('Consultare deschisă'));"
        "assert.ok(h.includes('public_consultation_opened'));"
        "assert.ok(h.includes('Notă existentă'));"
        "const noEvidence=mDrilldownHtml({...d,drilldown:{...d.drilldown,evidence_records:[]}});"
        "assert.ok(noEvidence.includes('data-workflow-empty-state'));"
        "assert.ok(noEvidence.includes('Dovezi citabile lipsă'));"
        "assert.ok(noEvidence.includes('Deschide sursele legate'));"
        "assert.ok(h.includes('Propunere existentă'));"
        "assert.ok(h.includes('rule-1'));"
        "assert.ok(!h.includes('<x>')&&!h.includes('<bad>')&&!h.includes('<img>')"
        "&&!h.includes('<script>')&&!h.includes('<mark>')&&!h.includes('<d>')&&!h.includes('<quote>')"
...
def test_matrix_workspace_queue_renders_daily_items_and_routes_actions():
    ...
    program = (
        ...
        "assert.ok(h.includes('Matrice zilnică de lucru'));"
        "assert.ok(h.includes('Pornește de aici'));"
        "assert.ok(h.includes('data-matrix-daily-buckets'));"
        "assert.ok(h.includes('gata de revizie'));"
        "assert.ok(h.includes('risc UE'));"
        ...
        "assert.ok(h.includes('Completează dovada înainte de concluzie.'));"
        "assert.ok(h.includes('notă locală · revizie umană · lege-1 · Loc art1'));"
        "assert.ok(h.includes('Dosar local'));"
        "assert.ok(h.includes('Alege dosar'));"
        "assert.ok(h.includes('Redactare'));"
        "assert.ok(h.includes('Regulă candidată'));"
        ...
        "const draftButtons=[{dataset:{matrixWorkspaceDraft:'Ministerul Test',"
        "matrixWorkspaceIndex:'0'},onclick:null}];"
        "const ruleButtons=[{dataset:{matrixWorkspaceRule:'Ministerul Test',"
        "matrixWorkspaceIndex:'0'},onclick:null}];"
        "const sourceButtons=[{dataset:{matrixWorkspaceSource:'camera'},onclick:null}];"
        "const bucketButtons=[{dataset:{matrixDailyBucket:'eu'},onclick:null}];"
        "const host={querySelectorAll:sel=>sel==='[data-matrix-workspace-action]'?[]:"
        "sel==='[data-matrix-daily-bucket]'?bucketButtons:"
        "sel==='[data-matrix-workspace-open-row]'?rowButtons:"
        "sel==='[data-matrix-workspace-open-dossier]'?dossierButtons:"
        "sel==='[data-matrix-workspace-draft]'?draftButtons:"
        "sel==='[data-matrix-workspace-rule]'?ruleButtons:"
        "sel==='[data-matrix-workspace-source]'?sourceButtons:[]};"
        "const library={open:false,scrollIntoView:o=>calls.push(['scroll','dossier',o.block])};"
        "const lifecycle={open:false,scrollIntoView:o=>calls.push(['scroll','lifecycle',o.block])};"
        "const ai={scrollIntoView:o=>calls.push(['scroll','ai',o.block])};"
        "const status={textContent:''};"
        "const nodes={'#m-workspace':host,'#dossier-library':library,'#dossier-status':status,"
        "'#lifecycle-tracker':lifecycle,'#lifecycle-status':status,'#ai-product-flow':ai,"
        "'#ai-product-flow-status':status,'#ue-text':{focus:()=>calls.push(['focus','ue'])}};"
        ...
        "function startManualNote(prefill,options){calls.push(['note',prefill,options]);}"
        "function reveal(sel,msg){const node=$(sel);"
        ...
        "draftButtons[0].onclick();"
        "let note=calls.find(c=>c[0]==='note');"
        "assert.equal(note[2].draft,true);"
        "assert.ok(note[1].title.includes('Redactare matrice'));"
        "assert.ok(note[1].reasoning.includes('coada zilnică a matricei'));"
        "ruleButtons[0].onclick();"
        "note=calls.filter(c=>c[0]==='note').at(-1);"
        "assert.equal(note[2].rule,true);"
        "assert.ok(note[1].title.includes('Regulă candidată matrice'));"
        "assert.ok(note[1].evidence_quote.includes('Tip: semnal matrice'));"
        "sourceButtons[0].onclick();"
        "assert.equal(lifecycle.open,true);"
        "bucketButtons[0].onclick();"
        "assert.ok(calls.some(c=>c[0]==='tab'&&c[1]==='ue'));"
    )
    result = subprocess.run(["node", "-e", program], capture_output=True, timeout=10)
    assert result.returncode == 0, result.stderr.decode()
...
def test_source_registry_renderer_escapes_and_labels_states():
    ...
    program = (
        ...
        "assert.ok(h.includes('data-source-note-action=\"src_1\"'));"
        "assert.ok(h.includes('Creează notă'));"
        ...
        "assert.ok(detail.includes('Creează notă din stare'));"
        "assert.ok(detail.includes('Istoric sync pentru această sursă'));"
        "assert.ok(detail.includes('achizitii_ue.v1'));"
        "const note=sourceRegistryNote({id:'src_1',family:'ue_cellar',identifier:'32014L0024',"
        "url:'https://example.test',label:'Directiva',state:'failed',last_error:'fetch_failed'});"
        "assert.equal(note.type,'risc_ue');"
        "assert.equal(note.status,'needs_evidence');"
        "assert.equal(note.source_url,'https://example.test');"
        "assert.ok(note.evidence_quote.includes('fetch_failed'));"
        "assert.ok(note.reasoning.includes('nu verdict juridic'));"
