...
def payload(**patch):
    data = {
        ...
        "confidence": "high",
        "extraction_method": "provision",
        "origin_kind": "provision",
        "origin_id": "ro:lege-98-2016#art7.alin2",
        "reviewer": "test",
    }
    data.update(patch)
    return data
...
def test_rule_candidate_is_reviewable_and_source_bound():
    ...
    assert out["applicability_scope"] == "proceduri de achiziție publică"
    assert out["confidence"] == "high"
    assert out["extraction_method"] == "provision"
    assert out["origin_kind"] == "provision"
    assert out["origin_id"] == "ro:lege-98-2016#art7.alin2"
...
def test_candidate_id_changes_for_reviewable_authoring_edits():
    first = rule_candidates.validate(payload(confidence="medium", reviewer="ana"))
    second = rule_candidates.validate(payload(confidence="high", reviewer="bogdan"))
