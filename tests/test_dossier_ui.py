...
def test_rule_drafts_panel_renders_queue_and_promoted_rules():
    ...
    code = (
        ...
        "assert.ok(html.includes('data-law-code-loop'));"
        "assert.ok(html.includes('Flux law-as-code'));"
        "assert.ok(html.includes('1. Revizuire candidat'));"
        "assert.ok(html.includes('2. Promovare regulă'));"
        "assert.ok(html.includes('3. Verificare deterministă'));"
        "assert.ok(html.includes('4. Notă de lucru'));"
        "assert.ok(html.includes('Totul rămâne candidat de lucru'));"
        "const blockedMsg='Verificarea pornește numai după promovarea unei ciorne';"
        "assert.equal(html.includes(blockedMsg),false);"
        "const blocked=lawCodeLoopStatusHtml({counts:{},total:0},{total:0},null);"
        "assert.ok(blocked.includes('blocată'));"
        "assert.ok(blocked.includes(blockedMsg));"
