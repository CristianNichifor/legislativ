...
| 7. Real-data acceptance | Bounded runtime path measured; current pack ready | The 2026-09-11 pilot release passed activation, search, workbench save as `fisa-act-v1` and private rollback survival. Missing/extraction/false-positive measurements remain incomplete. The current real pilot pack now has all five required ingredients ready, including official CELEX text and a reviewer-approved finding/proposal fixture. See V1_ACCEPTANCE.md. |
...
3 search results, 8 EU references signaled and rollback survival passed. It proves
the local/public runtime path, not legal coverage, EU-law assessment or proposal
correctness. Current readiness comes from `data/real_pilot_pack.json`, which now
has all five required gate ingredients ready.
